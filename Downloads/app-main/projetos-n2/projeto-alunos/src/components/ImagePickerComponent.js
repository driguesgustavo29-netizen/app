...
export default function ImagePickerComponent({ onImageSelected, label }) {
  ...
  const pickImage = async (useCamera) => {
    const result = useCamera ? await ImagePicker.launchCameraAsync({ mediaTypes: ['images'], quality: 1 }) : await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], quality: 1 });
